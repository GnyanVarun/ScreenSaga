...
class SeriesCommentsScreen extends StatefulWidget {
  final int seriesId;
  const SeriesCommentsScreen({super.key, required this.seriesId});

  @override
  State<SeriesCommentsScreen> createState() => _SeriesCommentsScreenState();
}

class _SeriesCommentsScreenState extends State<SeriesCommentsScreen> {
  ...
  void initState() {
    super.initState();
    comments = fetchComments(widget.seriesId);
  }

  Future<List<Map<String, dynamic>>> fetchComments(int tmdbId) async {
    final traktId = await getTraktIdFromTmdb(tmdbId);
    if (traktId == null) {
      throw Exception('Failed to find Trakt ID for series.');
    }

    final response = await http.get(
      Uri.parse('https://api.trakt.tv/shows/$traktId/comments'),
      headers: {
  ...
  Future<int?> getTraktIdFromTmdb(int tmdbId) async {
    ...
    final response = await http.get(
      Uri.parse('https://api.trakt.tv/search/tmdb/$tmdbId?type=show'),
      headers: {
        'Content-Type': 'application/json',
        'trakt-api-version': '2',
        'trakt-api-key': TraktAuth.clientId,
  ...
  Future<void> submitComment(String commentText) async {
    ...
    final traktId = await getTraktIdFromTmdb(widget.seriesId);
    final accessToken = await traktAuth.getAccessToken();
  ...
  Widget buildCommentTile(Map<String, dynamic> comment) {
    ...
    final username = user['username']?.toString().trim().isNotEmpty == true
        ? user['username']
        : 'Anonymous';
    final avatar = user['images']?['avatar']?['full'];
        ...
        children: [
          CircleAvatar(
            ...
            backgroundImage: avatar != null
                ? NetworkImage(avatar)
                : NetworkImage('https://walter.trakt.tv/images/default_avatar.png'),
            child: avatar == null
                ? Text(
              ...
              children: [
                ...
                Text(
                  ...
                  style: TextStyle(
                    ...
                    fontStyle: isSpoiler ? FontStyle.italic : FontStyle.normal,
                  ),
                ),
              ],
            ),
